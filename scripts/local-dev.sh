...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --fork <rpc-url>      Fork from a network (e.g., Base Sepolia)"
...
if [ -n "$FORK_URL" ]; then
    ...
    ANVIL_CMD="$ANVIL_CMD --fork-url $FORK_URL"
    if [ -n "$FORK_BLOCK" ]; then
        ANVIL_CMD="$ANVIL_CMD --fork-block-number $FORK_BLOCK"
    fi
else
    echo -e "${YELLOW}Starting fresh local chain (no fork)${NC}"
fi
...
DEPLOY_OUTPUT=$(forge script script/DeployLocal.s.sol:DeployLocal \
    ...
    --broadcast \
    --skip '*/TransferVerifier.sol' --skip '*/WithdrawVerifier.sol' \
    2>&1)

echo "$DEPLOY_OUTPUT"

# Extract addresses from output
TOKEN_ADDRESS=$(echo "$DEPLOY_OUTPUT" | grep "TestANON:" | awk '{print $2}')
...
NEXT_PUBLIC_TESTNET_ANON_TOKEN=$TOKEN_ADDRESS
