#!/bin/bash
# Wrapper script for withdraw e2e tests
...
TEST_DB_PATH="$ROOT_DIR/apps/web/data/test-withdraw.db"
TEST_PORT=3002
API_URL="http://localhost:$TEST_PORT"

# Pass through arguments (e.g., --client)
EXTRA_ARGS="$@"
...
echo ""
cd "$ROOT_DIR"
API_URL="$API_URL" bun run scripts/test-withdraw.ts $EXTRA_ARGS
TEST_EXIT_CODE=$?
