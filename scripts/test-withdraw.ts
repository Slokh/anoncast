 ...
 *
 * Usage:
 *   ./scripts/test-withdraw.sh          # Server-side proofs (default, ~7s)
 *   ./scripts/test-withdraw.sh --client # Client-side proofs (slower, ~70s)
 *   bun test:withdraw                    # Same as above
 */
...
console.log(
  `  Prover:     ${useServerMode ? "Server (native bb CLI)" : "Client (bb.js WASM)"}`,
);
console.log("");
...
async function main() {
  ...
  const commitmentBytes = pad(toHex(note.commitment), {
    size: 32,
  }) as `0x${string}`;
  ...
  const DEPOSIT_EVENT_SIGNATURE =
    "0xce80217ac10251ba83b6b6ce91da12bdd096aa99c37336c00bb01815ff8e77c1";
  ...
  const ourDepositInSession = sessionDeposits.find(
  ...
  console.log("");
  console.log(
    `Step 6: Generating ZK proof via ${useServerMode ? "server" : "client"}...`,
  );
  ...
  if (useServerMode) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Server proof generation failed: ${error.error || "Unknown error"}`,
      );
    }
    ...
    const circuit =
      await import("../packages/protocol/circuits/withdraw/target/anon_withdraw.json");
    const vkey =
      await import("../packages/protocol/circuits/withdraw/target/vk.json");
    const verifier = new WithdrawVerifier(
  ...
  const nullifierBytes = pad(toHex(nullifierHash), {
    size: 32,
  }) as `0x${string}`;
  ...
  console.log(
    `  Success:    ${balanceChange === depositAmount ? "YES ✓" : "NO ✗"}`,
  );
  console.log("");
